...
function search(query) {
	...
	var params = {
		part: 'snippet',
//		key: 'AIzaSyDAKDaBy_JDwcScSHqDQimOOLjdPImLanc', AIzaSyCOBqbpI5_-ePjfJ91Tbr1ElADByO57gEQ// github gist에서 본 api_token 이라서 새로 하나 받아야 할 것 같아요.
		key: 'AIzaSyBweoj3RJ-Uy1FBsRvWTMw_nB4mYIsBBmE',
		q: query,
		type: "video",
    maxResults: 20,
...
/**
 * Apply or remove grayscale (black & white)
 */
function grayscale(list, enable) {
  var selectedVideos = selectVideos(list);
  selectedVideos.forEach(video => {
    const iframe = video.getIframe();
    iframe.style.filter = enable ? "grayscale(100%)" : "none";
  });
}

/**
 * Adjust brightness and contrast
 * @param {float} brightness - 1 = normal, <1 darker, >1 brighter
 * @param {float} contrast - 1 = normal, >1 more contrast
 */
function adjustLight(list, brightness = 1, contrast = 1) {
  var selectedVideos = selectVideos(list);
  selectedVideos.forEach(video => {
    video.getIframe().style.filter = `brightness(${brightness}) contrast(${contrast})`;
  });
}

/**
 * Change hue (color tone)
 */
function hueRotate(list, degrees) {
  var selectedVideos = selectVideos(list);
  selectedVideos.forEach(video => {
    video.getIframe().style.filter = `hue-rotate(${degrees}deg)`;
  });
}
...
function blurVideo(list, amount) {
  var selectedVideos = selectVideos(list);
  selectedVideos.forEach(video => {
    video.getIframe().style.filter = `blur(${amount}px)`;
  });
}
...
function saturate(list, amount) {
  var selectedVideos = selectVideos(list);
  selectedVideos.forEach(video => {
    video.getIframe().style.filter = `saturate(${amount})`;
  });
}

/**
 * Apply a custom CSS filter string (combine multiple effects)
 */
function setFilter(list, filterString) {
  var selectedVideos = selectVideos(list);
  selectedVideos.forEach(video => {
    video.getIframe().style.filter = filterString;
  });
}
 ...
 */
function animateFilter(list, filterString, duration = 1.5) {
  var selectedVideos = selectVideos(list);
  selectedVideos.forEach(video => {
    const iframe = video.getIframe();
...
/**
 * Apply 3D tilt and scaling
 */
function tiltVideo(list, rotateX = 0, rotateY = 0, scale = 1) {
  var selectedVideos = selectVideos(list);
  selectedVideos.forEach(video => {
    const iframe = video.getIframe();
    iframe.style.transition = "transform 1s ease";
    iframe.style.transform = `rotateX(${rotateX}deg) rotateY(${rotateY}deg) scale(${scale})`;
  });
}

/**
 * Adjust opacity (fade visual intensity)
 */
function opacity(list, alpha = 1) {
  var selectedVideos = selectVideos(list);
  selectedVideos.forEach(video => {
    video.getIframe().style.opacity = alpha;
  });
}

/**
 * Visual presets for expressive effects
 */
const VisualPresets = {
 ...
 */
function applyPreset(list, name, animate = true) {
